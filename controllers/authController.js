...
exports.logout = async (req, res) => {
  try {
    ...
    if (token) {
      const payload = verifyRefreshToken(token);
      if (payload) {
        await User.updateOne({ _id: payload.id }, { $unset: { refreshToken: '' } });
      }
    }
...
exports.forgotPassword = async (req, res) => {
  ...
  try {
    ...
    if (!user || user.provider === 'google') {
      logger.warn(`Forgot password attempt on non-existent or Google account: ${email}`);
  ...
  try {
    ...
    if (currentPassword && newPassword) {
      if (user.provider === 'google' && !user.password)
        return res.status(400).json({
          message: 'Cannot set a password for an account created with Google.',
        });
      ...
      const uploadStream = cloudinary.uploader.upload_stream(
        ...
        (error, result) => {
          ...
          user
            ...
            .catch((err) => {
              res.status(500).json({ message: 'Server error on save', error: err.message });
            });
        }
      );
    ...
    logger.error(`Update profile error for user ${req.user.id}: ${err.message}`);
    res.status(500).json({ message: 'Server error', error: err.message });
  }
};
...
exports.googleLogin = async (req, res) => {
  ...
  try {
    ...
    if (!googleUser.email)
      return res.status(400).json({ message: 'Google account information is missing an email.' });
